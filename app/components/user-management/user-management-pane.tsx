...
export default function UserManagementPane({ users, count }: { users: UserData[]; count: number }) {
	...
	const resetUserPassword = async (userId: string) => {
		...
		if (!response.ok) {
			setUserUpdateError("Failed to send password reset email. Please try again.");
		} else {
			setUserUpdateError(null);
		}
	};
	...
	const handleChangeUserRole = async (userId: string, newRole: string) => {
		...
		if (!response.ok) {
			...
			setReactiveUsers((users) =>
				users.map((user) => (user.id === userId ? { ...user, role: updatedUser.user.role } : user)),
			);
		}
	};
	...
	const handlePageChange = (newPage: number) => {
		...
		fetch(`/api/user-management?page=${newPage}&limit=${process.env.NEXT_PUBLIC_ITEMS_PER_PAGE}`)
			.then((res) => res.json())
			.then((data) => {
		...
		<div className="flex flex-col gap-4 lg:items-start lg:justify-between">
			...
			<div className="flex w-full items-center justify-between">
				...
				>
					...
					<div className="mb-4">
						{userCreationError && <p className="text-sm text-red-500 mb-2">{userCreationError}</p>}
					</div>
				</Modal>
			...
			{userUpdateError && <p className="text-sm text-red-500 mb-2">{userUpdateError}</p>}
			...
			<PaginationControls
				totalPages={Math.ceil(reactiveCount / Number(process.env.NEXT_PUBLIC_ITEMS_PER_PAGE))}
